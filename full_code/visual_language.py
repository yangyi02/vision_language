...
from show_results import parse_log_and_draw
...
class VisualLanguage(object):
    ...
    def __init__(self, config):
        ...
        self.task_dir = os.path.join(self.cache_dir, 'task', self.task)
        self.task_dir += '_debug' if self.debug else ''
        self.task_dir = os.path.join(self.task_dir, self.image_feat_type)
        ...
        self.train_curve = os.path.join(self.model_dir, 'train')
        # Make directories
    ...
    def train(self):
        ...
        if not os.path.exists(self.train_log):
            ...
            #           % (self.train_conf, False, self.trainer_count,
    ...
    def draw_error_curve(self):
        ...
        """
        if not os.path.exists(self.train_curve):
            parse_log_and_draw.main(self.train_log, self.train_curve, self.task)
        logging.info('Train error curves save to %s', self.train_curve)
    ...
    def send_email(self):
        ...
        if self.email == '':
            return
        message = io.open(self.train_log).readlines()[-6:]
        subject, to_address, from_address, server, attach_file = self.model_dir, self.email, \
            self.email, 'hotswap-in.baidu.com', self.train_log
        cmd = './sendEmail -u %s -m %s -t %s -f %s -s %s -a %s' \
            % (subject, str(message), to_address, from_address, server, attach_file)
        os.system(cmd)
        logging.info('Email send to %s', to_address)
