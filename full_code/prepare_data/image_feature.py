...
import sys
import logging
from prepare_data import extract_vgg, extract_google, extract_resnet, extract_resnet152, extract_resnet152_spatial, utils
...
class ImageFeature(object):
    ...
    def extract_feature(self):
        ...
        if not os.path.exists(self.image_feat_batch_dir):
            if self.image_feat_type == 'vgg_fc7_4096':
                extract_vgg.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=False)
            elif self.image_feat_type == 'vgg_fc7_4096_oversample':
                extract_vgg.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=True)
            elif self.image_feat_type == 'google_pool5_1024':
                extract_google.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=False)
            elif self.image_feat_type == 'google_pool5_1024_oversample':
                extract_google.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=True)
            elif self.image_feat_type == 'resnet_pool5_2048_oversample':
                extract_resnet.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=True)
            elif self.image_feat_type == 'resnet152_pool5_2048_oversample':
                extract_resnet152.extract_feature(self.image_list, self.image_feat_batch_dir, oversample=True)
            elif self.image_feat_type == 'resnet152_res5c_spatial_2048':
                extract_resnet152_spatial.extract_feature(self.image_list, self.image_feat_batch_dir)
            else:
