...
def extract_feature(image_list, feature_dir, oversample=True):
    ...
    pretrained_model = path.join(caffe_root, 'models/bvlc_googlenet/bvlc_googlenet.caffemodel')
    model_def = path.join(caffe_root, 'models/bvlc_googlenet/deploy.prototxt')
    ...
    for num_file in xrange(len(image_files)):
        ...
        out = net.forward_all(**{'data': caffe_in, 'blobs': ['pool5/7x7_s1']})
        predictions = out['pool5/7x7_s1']
        if oversample:
