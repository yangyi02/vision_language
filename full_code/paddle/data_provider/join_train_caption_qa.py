...
import numpy
import random
from PyDataProviderWrapper import DenseSlot, IndexSlot, provider
...
def processData(obj, file_name):
    ...
    for sample in data['data']:
        ...
        if coding_q[0] != obj.word_dict.get('$$S$$', -1):
            coding_q.insert(0, obj.word_dict.get('$$S$$', -1))
        if coding_q[-1] != obj.word_dict.get('$$E$$', -1):
            coding_q.append(obj.word_dict.get('$$E$$', -1))
        coding_current_word = coding_a[:-1]
        coding_next_word = coding_a[1:]
        coding_current_ques = coding_q[:-1]
        coding_next_ques = coding_q[1:]
        yield [img_feat], coding_current_ques, coding_next_ques, coding_current_word, coding_next_word
    # end_time = time.time()
