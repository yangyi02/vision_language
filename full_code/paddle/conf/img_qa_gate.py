...
MixedLayer(name = 'gated_question',
    size = hidden_dim/4,
    bias = False,
    inputs = DotMulOperator(['question_gate_expanded', 'question_expand'])
)

MixedLayer(name = 'gated_img',
    size = img_feat_dim,
    bias = False,
    inputs = DotMulOperator(['image_gate_expanded', 'img_expand'])
)
...
MixedLayer(name = 'hidden2',
    size = multimodal_dim,
    active_type = 'stanh',
    inputs = [FullMatrixProjection('gated_question'),
        FullMatrixProjection('word_embedding'),
        FullMatrixProjection('gated_img',
                            learning_rate = 0.01,
                            momentum = 0.9,
                            decay_rate = 0.05,
                            initial_std = 0.01),
