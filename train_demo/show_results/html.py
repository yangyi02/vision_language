...
class HtmlQA(Html):
    def make_html(self):
        ...
        handle.write('<tr><td>Question</td><td>Answer</td><td>Ground truth</td></tr>\n')
        for question_id in list(set(question_ids)):
            handle.write(self.wrap_text(question_dict[question_id], answer_dict[question_id],
                                              gt_answer_dict[question_id]))
        handle.write('</table>\n</body>\n</html>\n')
        logging.info('Html results save to %s', self.html_file)

    @staticmethod
    def wrap_text(question, answer, gt_answer):
        """
